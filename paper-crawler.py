...
def get_paper_list(page_soup, page_type):
    if page_type == 'icml_jmlr_hosted' or page_type == 'icml2012' or page_type == 'icml2010' or page_type == 'icml2011'  or page_type == 'icml2009':
...
def get_paper_data(paper, page_type, page_address):
    ...
    if page_type == 'icml_jmlr_hosted':
        ...
        abstract_data = clean_text(paper_page_soup.find('div', id='abstract').text)

    elif page_type == 'jmlr_volume':
        paper_page_request = requests.get(urljoin(page_address, paper.find('a', text='abs').attrs['href']))
        paper_page_soup = BeautifulSoup(sub(r'(<font color=)', r'</div>\1', sub(r'(<h3>Abstract<\/h3>)', '\1<div id="abstract">', paper_page_request.content.decode(paper_page_request.encoding))), 'html.parser')
        paper_file_name = paper.find('a', text='pdf').attrs['href'].split('/')[-1]
        title = clean_text(paper.find('dt').next_element)
        authors = paper.find('dd').find('i').text
        web_link = urljoin(page_address, paper.find('a', text='abs').attrs['href'])
        pdf_link = urljoin(page_address, paper_page_soup.find('a', text='pdf').attrs['href'])
        abstract_data = clean_text(paper.find('div', id='abstract'))

    elif page_type == 'icml2012':
        title = paper.find('h2').text
        authors = clean_text(paper.find('p', {'class': 'authors'}).next_element)
        ...
        title = clean_text(paper.find('h3').text)
        authors = clean_text(paper.find('span', {'class': 'name'}).text)
        ...
        abstract_data = clean_text(next(islice(paper.find('p').next_elements, 2, None)))
        ...
        paper_file_name = paper.find('a', text='Full paper').attrs['href'].split('/')[-1]
        pdf_link = urljoin(page_address, paper.find('a', text='Full paper').attrs['href'])
        abstract_data = clean_text(paper.find('a', text='Full paper').find_previous('p').text)
        ...
        pdf_link = urljoin(page_address, paper_page_soup.find('a', text='[PDF]').attrs['href'])
        abstract_data = paper_page_soup.find('p', {'class': 'abstract'}).text.replace('Abstract missing', '')

    return {'title': title, 'authors': authors, 'web_link': web_link, 'paper_file_name': paper_file_name, 'pdf_link': pdf_link, 'abstract_data': abstract_data, 'source': source, 'year': year}
...
with open(csv_file_name, 'w', newline='', encoding='utf-8') as csv_file:
    ...
    for page_group in [pages_to_crawl[4]]:
...
# - HTML entities in ICML author names
# - Difference between requests.get().text and requests.get().content.decode(requests.get().encoding)?
# - Prevent single papers in ICML 2011 from having the complete rest of the document attached (performance)